...
public class TodoTxtStepDefs implements En {
    ...
    public TodoTxtStepDefs() {
        ...
        When("^I list the todo items with no arguments$", () -> items = new ListFunc(file, Main.EVENT_BUS).list());
        Then("^I will get a list with (\\d+) items$", (Integer num) -> assertThat(items.size(), is(equalTo(num))));
        When("^I list the todo items with arguments \"([^\"]*)\"$", (String args) -> items = new ListFunc(file, Main.EVENT_BUS).list(Arrays.asList(args.split(" "))));
        ...
        When("^I complete todo item (\\d+)$", (Integer idx) -> new DoneFunc(file, Main.EVENT_BUS).done(idx));
        ...
        When("^I complete todo item (\\d+) (\\d+) (\\d+) (\\d+)$", (Integer arg0, Integer arg1, Integer arg2, Integer arg3) -> {
            ...
            new DoneFunc(file, Main.EVENT_BUS).done(args);
        });
        ...
        When("^I list the priority tasks$", () -> items = new ListFunc(file, Main.EVENT_BUS).listPriority());
        When("^I list the contexts$", () -> items = new ListFunc(file, Main.EVENT_BUS).listContexts());
        When("^I list the projects$", () -> items = new ListFunc(file, Main.EVENT_BUS).listProjects());
        When("^I list everything$", () -> items = new ListFunc(file, Main.EVENT_BUS).listAll());
        When("^I remove item (\\d+) (\\d+) (\\d+) (\\d+)$", (Integer arg0, Integer arg1, Integer arg2, Integer arg3) -> {
            List<Integer> args = Arrays.asList(arg0, arg1, arg2, arg3);
            new RemoveFunc(file, Main.EVENT_BUS).remove(args);
        });
        When("^I replace item (\\d+) with \"([^\"]*)\"$", (Integer idx, String todoItem) -> new ReplaceFunc(file, Main.EVENT_BUS).replace(idx, todoItem));
        And("^item (\\d+) will contain \"([^\"]*)\"$", (Integer idx, String todoItem) -> assertThat(items.get(idx -1), containsString(todoItem)));
        When("^I add priority \"([^\"]*)\" to item (\\d+)$", (String priority, Integer idx) -> new PriorityFunc(file, Main.EVENT_BUS).add(idx, priority));
        When("^I remove priority from item (\\d+)$", (Integer idx) -> new PriorityFunc(file, Main.EVENT_BUS).remove(idx));
    }
}
